...
add_nat_rule() {
  local public_port="$1"
  local target_port="${2:-$1}"
  sudo iptables -t nat -C PREROUTING -p tcp --dport "$public_port" -j DNAT --to-destination "${FLOATING_IP}:${target_port}" 2>/dev/null \
    || sudo iptables -t nat -A PREROUTING -p tcp --dport "$public_port" -j DNAT --to-destination "${FLOATING_IP}:${target_port}"
}
...
echo "OpenStack resources and hybrid iptables NAT configured."
